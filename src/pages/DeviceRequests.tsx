...
const SUMMARY_STATUSES: RequestApprovalStatus[] = [
  'Requested',
  'Pending',
  'Approved',
  'Rejected',
...
export default function DeviceRequests() {
  ...
  const canCreateDeviceRequest =
    canCreateDeviceRequestForUser(currentUser?.email)

  const access = getUserAccess(currentUser?.email)

  const isRestrictedUserFlag =
    access.isRestricted && !access.canCreateRequest

  const canViewRecommended = currentUser?.email
    ? canViewRecommendedStatus(currentUser.email)
    : true

  const canViewRejected = currentUser?.email
    ? canViewRejectedStatus(currentUser.email)
    : true

  const canViewFulfilled = currentUser?.email
    ? canViewFulfilledStatus(currentUser.email)
    : true

  const canViewRequested = currentUser?.email
    ? canViewRequestedStatus(currentUser.email)
    : true

  const canDeleteRequest = currentUser?.email
    ? canDeleteDeviceRequest(currentUser.email)
    : true
  ...
  const [form, setForm] =
    useState<DeviceRequestFormData>(emptyForm)
  ...
  const [lookupErrorMessage, setLookupErrorMessage] =
    useState<string | null>(null)
  ...
  /**
   * Subscribe to device request changes.
   */
  useEffect(() => {
    const unsubscribe =
      subscribeToDeviceRequestsChanged(() =>
      ...
      try {
        ...
        setErrorMessage(
          error instanceof Error
            ? error.message
            : 'Unable to load device requests.'
        )
      } finally {
  ...
  const refreshRequests = async () => {
    ...
    try {
      const requestItems = await fetchDeviceRequests({
        approvalStatus: '',
        deviceType: '',
      })
   ...
   *
   * IMPORTANT:
   * DeviceRequestListItem does NOT contain updatedAt.
   *
   * Therefore we use the actual workflow dates:
   * - requestDate
   * - recommendedDate
   * - approvalDate
   * - fulfilledDate
   *
   * This allows the date filter to recognize a request
   * when it has moved to another Kanban status.
   */
    ...
    return requests
      .filter((request) => {
        ...
        const matchesDate = (() => {
          ...
          /**
           * Collect all valid workflow dates.
           *
           * The type guard:
           * (date): date is string
           *
           * tells TypeScript that after filtering,
           * every remaining value is a string.
           */
          const possibleDates = [
            request.requestDate,
          ...
          // If there are no valid dates, don't hide the request.
          if (possibleDates.length === 0) {
            return true
          }

          // Use the most recent workflow activity date.
          const latestTimestamp = Math.max(...possibleDates)

          /**
           * Compare using YYYY-MM-DD strings instead of
           * Date objects where possible to avoid timezone
           * shifting.
           */
  ...
  const handleApprove = async (
    ...
    try {
      const updatedRequest =
        await approveDeviceRequestById(
          requestId,
          {
  ...
  const handleCreate = async () => {
    ...
    if (
      ...
      showToast(
        'Requester, department, requested for, device type, and reason are required',
        'error'
      )
      return
    }
  ...
  /**
   * Users who should not see Approve / Reject buttons.
   */
  ...
  /**
   * Determine which summary cards are visible.
   */
  const visibleSummaryStatuses =
    SUMMARY_STATUSES.filter(
      (status) => {
        if (
          status === 'Requested' &&
          !canViewRequested
  ...
  const summaryGridClass =
    visibleSummaryStatuses.length === 3
      ? 'grid-cols-1 sm:grid-cols-3'
      : visibleSummaryStatuses.length === 4
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      {/* Summary cards */}
      <div
        className={`grid ${summaryGridClass} gap-3`}
      >
        {visibleSummaryStatuses.map(
          (status) => (
            <div
              key={status}
                  ...
                  (request) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <StatusBadge
                            status={formatDeviceRequestStatus(
                              request.approvalStatus
                            )}
                          />
                        </td>
