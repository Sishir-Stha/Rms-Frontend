...
import {
  DEPARTMENT_REQUESTS_DATA,
  MONTHLY_REPAIRS,
  TECHNICIAN_PERFORMANCE,
} from '../data/dummyData'

type ReportId = 'monthly-repair' | 'device-request' | 'technician-performance'
...
function CustomTooltip({ active, payload, label }: CustomTooltipProps) {
  ...
  return (
    <div
      className="rounded-xl px-4 py-3 shadow-ambient"
      style={{ background: '#2d3449', border: '1px solid rgba(62,74,61,0.3)' }}
    >
...
function MonthlyRepairReport() {
  ...
  const pageSize = 6
  const totalPages = Math.max(1, Math.ceil(MONTHLY_REPAIRS.length / pageSize))
  const pagedRows = MONTHLY_REPAIRS.slice((page - 1) * pageSize, page * pageSize)

  return (
    <div className="space-y-6">
      <div className="section-card">
        <h4 className="font-semibold text-sm text-on-surface mb-4">
          Repairs vs Completed - 12 Months
        </h4>
        <ResponsiveContainer width="100%" height={240}>
          <AreaChart data={MONTHLY_REPAIRS}>
            <defs>
              <linearGradient id="rg1" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#62df7d" stopOpacity={0.3} />
                <stop offset="95%" stopColor="#62df7d" stopOpacity={0} />
              </linearGradient>
      ...
      <div className="section-card">
        ...
        <table className="data-table w-full">
          ...
          <tbody>
            {pagedRows.map((month) => (
              <tr key={month.month}>
                <td className="font-medium text-on-surface">{month.month}</td>
                <td className="text-on-surface">{month.repairs}</td>
                <td><span className="text-primary font-semibold">{month.completed}</span></td>
                <td><span className="text-warning font-semibold">{month.pending}</span></td>
                <td><span className="text-on-surface">{Math.round((month.completed / month.repairs) * 100)}%</span></td>
              </tr>
            ))}
          </tbody>
        </table>
        <div className="flex items-center justify-between pt-4 mt-2 border-t border-outline-variant/15">
          <p className="text-xs text-on-surface-variant">
            {pagedRows.length} of {MONTHLY_REPAIRS.length} rows
          </p>
          <div className="flex gap-1">
            <button
              disabled={page === 1}
              onClick={() => setPage((currentPage) => currentPage - 1)}
              className="w-7 h-7 btn-ghost rounded-lg disabled:opacity-30"
            >
              <ChevronLeft size={14} />
            </button>
            <span className="text-xs px-2 text-on-surface">
              {page}/{totalPages}
            </span>
            <button
    ...
    <div className="space-y-6">
      ...
      <div className="section-card">
        ...
        <table className="data-table w-full">
          ...
          <tbody>
            {DEPARTMENT_REQUESTS_DATA.map((department) => (
              <tr key={department.dept}>
                <td className="font-medium text-on-surface">{department.dept}</td>
                <td>{department.requests}</td>
                <td className="text-primary font-semibold">{department.approved}</td>
                <td className="text-error font-semibold">{department.rejected}</td>
                <td className="text-warning font-semibold">{department.pending}</td>
                <td>{Math.round((department.approved / department.requests) * 100)}%</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="space-y-6">
      ...
      <div className="section-card">
        ...
        <table className="data-table w-full">
          ...
          <tbody>
            {TECHNICIAN_PERFORMANCE.map((technician) => (
              <tr key={technician.name}>
                <td className="font-medium text-on-surface">{technician.name}</td>
                <td className="text-primary font-semibold">{technician.resolved}</td>
                <td className="text-secondary">{technician.inProgress}</td>
                <td className="text-on-surface">{technician.avgDays} days</td>
                <td>
...
export default function Reports() {
  ...
  if (selected) {
    const report = REPORT_LIST.find((item) => item.id === selected)
    ...
    <div className="p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-on-surface-variant">
          Select a report to view detailed analytics and data
        </p>
      </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-4">
                    ...
                    <span className="text-xs font-semibold" style={{ color: report.color }}>
