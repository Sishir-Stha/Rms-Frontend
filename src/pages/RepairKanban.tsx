...
import { fetchRepairs, moveRepairCard, subscribeToRepairsChanged } from '../services/repair.service'
...
export default function RepairKanban() {
  ...
  useEffect(() => {
    ...
    const loadRepairs = async () => {
      ...
      try {
        const response = await fetchRepairs({ status: '', device_name: '' }, abortController.signal)
  ...
  const onDragOver = (event: DragEvent<HTMLDivElement>, colId: RepairStatus) => {
  ...
  const moveCard = (id: string, status: RepairStatus) => {
    ...
    void (async () => {
      try {
        ...
        showToast(error instanceof Error ? error.message : 'Failed to move repair.', 'error')
      }
    })()
  }

  const onDrop = (event: DragEvent<HTMLDivElement>, colId: RepairStatus) => {
    event.preventDefault()
    if (!dragging) return
  ...
  const columnsWithCards = useMemo(() => COLUMNS.map((column) => ({ ...column, cards: cards.filter((card) => card.status === column.id) })), [cards])
    ...
    <div className="p-6">
      <div className="mb-5 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--on-surface-variant)' }}>Backend-driven repair board grouped by status</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="section-card flex flex-col items-center justify-center min-h-[360px] text-center px-6">
          <p className="text-base font-semibold" style={{ color: 'var(--error)' }}>Unable to load repair board</p>
        ...
        <div className="section-card flex flex-col items-center justify-center min-h-[360px] text-center px-6">
          <p className="text-base font-semibold text-on-surface">No records found for this status</p>
        </div>
      ) : (
          ...
          {columnsWithCards.map((column) => {
            ...
            return (
              <div
                ...
                style={{ width: '280px', background: 'var(--kanban-col-bg)', border: '1px solid var(--border-color)', borderTop: `3px solid ${column.color}` }}
              ...
              >
                ...
                <div className="flex-1 p-3 space-y-3 transition-colors" style={{ minHeight: '200px', background: dragOver === column.id ? 'var(--kanban-col-hover)' : undefined }}>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between" style={{ borderTop: '1px solid var(--border-color)', paddingTop: '8px' }}>
                        <div className="flex items-center gap-1.5">
                          <div className="w-5 h-5 rounded-full text-xs flex items-center justify-center font-bold" style={{ background: 'rgba(98,223,125,0.15)', color: '#16a34a' }}>
                            {card.reportedBy.split(' ').map((name) => name[0]).join('') || '?'}
                          </div>
                  ...
                  {columnCards.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-8 text-xs text-center opacity-40" style={{ color: 'var(--on-surface-variant)' }}>
                      <p>Drop cards here</p>
                    </div>
                  )}
                </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { if (confirm) moveCard(confirm.id, confirm.col) }}
