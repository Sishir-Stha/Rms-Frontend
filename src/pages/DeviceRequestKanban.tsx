...
export default function DeviceRequestKanban() {
  ...
  /**
   * Returns the correct date for the current Kanban status.
   *
   * Requested   -> requestDate
   * Recommended -> recommendedDate
   * Approved    -> approvalDate
   * Rejected    -> approvalDate
   * Fulfilled   -> fulfilledDate
   */
  const getCardStatusDate = useCallback(
    (card: DeviceRequestListItem): string => {
      switch (card.approvalStatus) {
        ...
        case 'Pending':
          return (
            getLocalDateString(card.recommendedDate) ||
            getLocalDateString(card.requestDate)
          )

        case 'Approved':
        case 'Rejected':
          return (
            getLocalDateString(card.approvalDate) ||
            getLocalDateString(card.requestDate)
          )

        case 'Fulfilled':
          return (
            getLocalDateString(card.fulfilledDate) ||
            getLocalDateString(card.approvalDate) ||
            getLocalDateString(card.requestDate)
          )
  ...
  const visibleColumns = useMemo(() => {
    ...
    currentUser?.email,
    access.canViewRequested,
    access.canViewRecommended,
    access.canViewRejected,
    access.canViewFulfilled,
  ])
      ...
      try {
        ...
        if (!signal?.aborted) {
          ...
          const filteredCards = requestCards.filter((card) => {
            ...
            /*
             * Use the date belonging to the current Kanban status.
             *
             * This is important because Recommended and Fulfilled
             * now have their own date fields.
             */
            ...
            if (filterMode === 'default') {
              if (
                ...
                card.approvalStatus === 'Fulfilled'
              ) {
                if (cardDate && cardDate < oneMonthAgoStr) {
                  return false
                }
              }
            }
  ...
  const onDragOver = (
    ...
    if (dragging !== null) {
      ...
      if (draggedCard) {
        const allowed =
          VALID_TRANSITIONS[draggedCard.approvalStatus] ?? []
  ...
  const updateCard = (updatedCard: DeviceRequestListItem) => {
    setCards((previousCards) =>
      previousCards.map((card) =>
        card.requestId === updatedCard.requestId
          ? updatedCard
          : card
      )
    )
  ...
  const moveCard = async (
    ...
    const currentStatus =
      fromStatus || targetCard.approvalStatus
     ...
     * Optimistic update.
     *
     * Update the date field belonging to the destination status
     * immediately, so the UI shows today's date without waiting
    ...
    switch (col) {
      ...
      case 'Approved':
        optimisticCard = {
          ...
          approvalStatus: col,
          approvedById:
            currentUser?.id ?? targetCard.approvedById,
          approvedBy:
            currentUser?.name ?? targetCard.approvedBy,
    ...
    try {
      /*
       * Approved / Rejected continue using the existing
       * approval endpoint.
       *
      ...
      if (col === 'Approved' || col === 'Rejected') {
        ...
        const updatedCard = await approveDeviceRequestById(
          requestId,
          {
            approval_status: col,
      ...
      /*
       * Requested / Recommended / Fulfilled use the
       * generic Kanban movement endpoint.
       *
       * Backend now updates:
       *
       ...
       */
      const updatedCard = await moveDeviceRequestCard(
        requestId,
        col
      )
  ...
  const onDrop = async (
    ...
    if (!canManageKanban(currentUser?.email)) {
      showToast(
        'You do not have permission to move requests',
        'error'
      )
      ...
      >
        {visibleColumns.map((column) => {
          const columnCards = cards.filter(
            (card) => card.approvalStatus === column.id
          )
                ...
                {columnCards.map((card) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {/*
                       * IMPORTANT:
                       * Display the date belonging to the
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (confirm) {
            const targetCard = cards.find(
              (card) =>
                card.requestId === confirm.requestId
            )
