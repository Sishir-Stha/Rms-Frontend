...
import { AlertCircle, ArrowLeft, Building, Calendar, CheckCircle2, ChevronRight, Clock, FileText, Hash, Monitor, Save, Tag, User, XCircle, type LucideIcon } from 'lucide-react'
...
const formatDateInputValue = (value: string | null): string => value ? value.slice(0, 10) : ''
const toNullableDate = (value: string): string | null => value.trim() === '' ? null : value
const parseRequestId = (value: string | undefined): number | null => { if (!value) return null; const parsed = Number(value); return Number.isInteger(parsed) ? parsed : null }

const createRequestForm = (request: DeviceRequestDetailItem): RequestDetailFormState => ({
  requestId: request.requestId, id: request.id, requestedById: request.requestedById, requestedFor: request.requestedFor, departmentId: request.departmentId, deviceType: request.deviceType, brand: request.brand, reason: request.reason, requestDate: formatDateInputValue(request.requestDate), approvalStatus: request.approvalStatus, approvedById: request.approvedById, approvedByName: request.approvedBy, approvalDate: formatDateInputValue(request.approvalDate), priority: request.priority, quantity: request.quantity,
})
...
export default function RequestDetail() {
  ...
  if (isLoading) {
    return <div className="p-6 flex flex-col items-center justify-center" style={{ minHeight: '60vh' }}><div className="w-10 h-10 rounded-full border-2 border-primary border-t-transparent animate-spin" /><p className="text-sm mt-4" style={{ color: 'var(--muted)' }}>Loading request detail...</p></div>
  }
  ...
  const handleSave = () => {
    ...
    void (async () => {
      try {
        await updateDeviceRequestById(form.requestId, { requested_by: form.requestedById, department_id: form.departmentId, device_type: form.deviceType.trim(), brand: form.brand.trim(), reason: form.reason.trim(), quantity: form.quantity, priority: form.priority, requested_for: form.requestedFor.trim(), request_date: toNullableDate(form.requestDate), approval_status: form.approvalStatus, approved_by: form.approvedById, approval_date: toNullableDate(form.approvalDate) })
  ...
  const handleApproval = (nextStatus: 'Approved' | 'Rejected' | 'Fulfilled' | 'Pending') => {
    if (!currentUser) {
    ...
    void (async () => {
      try {
        ...
        setRequests(requestItems)
        setForm(createRequestForm(requestDetail))
        let toastMsg = `${form.id} ${nextStatus.toLowerCase()}`
        let toastType: 'success' | 'info' = 'info'
        if (nextStatus === 'Approved' || nextStatus === 'Fulfilled' || nextStatus === 'Pending') {
          toastMsg = nextStatus === 'Pending' ? `${form.id} recommended` : `${form.id} ${nextStatus.toLowerCase()}`
          toastType = 'success'
        }
        showToast(toastMsg, toastType)
      } catch (error) {
        showToast(error instanceof Error ? error.message : `Failed to mark request as ${nextStatus}.`, 'error')
      } finally {
  ...
  // STRICT CHECK: Only allow review if explicitly permitted for the current status
    ...
    <div className="p-6 animate-fade-in">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {canReviewFinal && (
            <>
              <button onClick={() => setShowRejectDialog(true)} className="btn-ghost px-3 py-1.5 rounded-lg text-sm flex items-center gap-1.5" style={{ color: 'var(--error-text)', border: '1px solid var(--border-strong)' }}><XCircle size={14} /> Reject</button>
              <button onClick={() => {
                const targetStatus = form.approvalStatus === 'Requested' && requestedActionLabel === 'Recommend' ? 'Pending' : 'Approved'
                handleApproval(targetStatus)
              }} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold" style={{ background: 'rgba(0,83,56,0.1)', color: 'var(--success-text)', border: '1px solid rgba(0,83,56,0.2)' }}>
                <CheckCircle2 size={14} /> {form.approvalStatus === 'Requested' ? requestedActionLabel : 'Approve'}
              </button>
            </>
          )}

          {canFulfillRequest && form.approvalStatus === 'Approved' && (
            <button onClick={() => handleApproval('Fulfilled')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-semibold" style={{ background: 'rgba(0,83,56,0.1)', color: 'var(--success-text)', border: '1px solid rgba(0,83,56,0.2)' }}>
              <CheckCircle2 size={14} /> Fulfill
            </button>
          )}

          {canSave && (
            <button onClick={handleSave} className="btn-primary text-sm" disabled={isSaving}><Save size={14} /> {isSaving ? 'Saving...' : 'Save Changes'}</button>
          )}
        </div>
      </div>
        ...
        <div className="xl:col-span-2 space-y-5">
          <div className="section-card">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div><label className={FIELD_LABEL} style={{ color: 'var(--muted)' }}>Priority</label><select value={form.priority} onChange={(e) => setField('priority', e.target.value as Priority)} disabled={!deviceDetailsEditable} className="input-field">{PRIORITIES.map((p) => <option key={p} value={p}>{p}</option>)}</select></div>
              <div><label className={FIELD_LABEL} style={{ color: 'var(--muted)' }}>Quantity</label><input type="number" min="1" value={form.quantity} onChange={(e) => setField('quantity', Math.max(1, Number(e.target.value) || 1))} disabled={!deviceDetailsEditable} className="input-field" /></div>
          ...
          <div className="section-card" style={{ background: 'var(--primary-lighter)', border: '1px solid var(--primary-light)' }}>
            ...
            <div className="flex items-start gap-3 pt-2.5">
              ...
              <div>
                ...
                <div className="mt-1"><StatusBadge status={formatDeviceRequestStatus(form.approvalStatus)} /></div>
              </div>
            </div>
          </div>
